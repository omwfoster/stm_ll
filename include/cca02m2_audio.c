...
/* PDM filters params */
static PDM2PCM_Handler_t PDM2PCMHandler[4];
    ...
    else if (Instance == 1U)
    {
      ...
      for (i = 0; i < DFSDM_MIC_NUMBER; i ++)
      {
        ...
        if (((AudioInit->Device >> (uint8_t)i) & AUDIO_IN_DIGITAL_MIC1) == AUDIO_IN_DIGITAL_MIC1)
        {
          ...
          if (HAL_DFSDM_FILTER_RegisterCallback(&hAudioInDfsdmFilter[i], HAL_DFSDM_FILTER_REG_COMPLETE_CB_ID,
                                                DFSDM_FilterRegConvCpltCallback) != HAL_OK)
          {
            return  BSP_ERROR_PERIPH_FAILURE;
          }
          if (HAL_DFSDM_FILTER_RegisterCallback(&hAudioInDfsdmFilter[i], HAL_DFSDM_FILTER_REG_HALFCOMPLETE_CB_ID,
  ...
  if (Instance == 1U)
  {
    for (i = 0; i < DFSDM_MIC_NUMBER; i ++)
    {
      if (((AudioInCtx[Instance].Device >> i) & AUDIO_IN_DIGITAL_MIC1) == AUDIO_IN_DIGITAL_MIC1)
      {
        ...
        if (HAL_DFSDM_CHANNEL_RegisterCallback(&hAudioInDfsdmChannel[i], HAL_DFSDM_CHANNEL_MSPINIT_CB_ID,
                                               DFSDM_ChannelMspInit) != HAL_OK)
        {
          return BSP_ERROR_PERIPH_FAILURE;
        }
        else if (HAL_DFSDM_FILTER_RegisterCallback(&hAudioInDfsdmFilter[i], HAL_DFSDM_FILTER_MSPINIT_CB_ID,
                                                   DFSDM_FilterMspInit) != HAL_OK)
        {
          return BSP_ERROR_PERIPH_FAILURE;
        }
        else if (HAL_DFSDM_CHANNEL_RegisterCallback(&hAudioInDfsdmChannel[i], HAL_DFSDM_CHANNEL_MSPDEINIT_CB_ID,
                                                    DFSDM_ChannelMspDeInit) != HAL_OK)
        {
          return BSP_ERROR_PERIPH_FAILURE;
        }
        else if (HAL_DFSDM_FILTER_RegisterCallback(&hAudioInDfsdmFilter[i], HAL_DFSDM_FILTER_MSPDEINIT_CB_ID,
  ...
  else
  {
    if (Instance == 1U)
    {
      ...
      for (i = 0U; i < DFSDM_MIC_NUMBER; i ++)
      {
        ...
        if (HAL_DFSDM_FILTER_RegisterCallback(&hAudioInDfsdmFilter[i], HAL_DFSDM_FILTER_MSPINIT_CB_ID,
        ...
        else if (HAL_DFSDM_CHANNEL_RegisterCallback(&hAudioInDfsdmChannel[i], HAL_DFSDM_CHANNEL_MSPINIT_CB_ID,
                                              ...
                                              uint32_t ChnlNbrOut)
{
  ...
  else
  {
    ...
    for (index = 0; index < ChnlNbrIn; index++)
    {
      ...
      /* Init PDM filters */
      PDM2PCMHandler[index].bit_order  = PDM2PCM_BIT_ORDER_MSB;
      if (ChnlNbrIn == 1U)
      {
        PDM2PCMHandler[index].endianness = PDM2PCM_ENDIANNESS_BE; /* For WB this should be LE, TODO after bugfix in PDMlib */
      }
      else
      {
        PDM2PCMHandler[index].endianness = PDM2PCM_ENDIANNESS_LE;
      }
      ...
      switch (AudioInCtx[0].DecimationFactor)
      {
        ...
        case 64:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_64_HI_PERF;
          break;
        case 96:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_96_HI_PERF;
          break;
#else
        case 16:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_16;
          break;
        case 24:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_24;
          break;
        case 32:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_32;
          break;
        case 48:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_48;
          break;
        case 64:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_64;
          break;
        case 80:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_80;
          break;
        case 128:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_128;
          break;
        case 160:
          PDM2PCMConfig[index].decimation_factor = PDM2PCM_DEC_FACTOR_80;
      ...
      switch (AudioInCtx[0].BitsPerSample)
      {
        case AUDIO_RESOLUTION_16b:
          PDM2PCMConfig[index].bit_depth = PDM2PCM_BITDEPTH_16;
          break;
        case AUDIO_RESOLUTION_24b:
          PDM2PCMConfig[index].bit_depth = PDM2PCM_BITDEPTH_24;
          break;
        case AUDIO_RESOLUTION_32b:
          PDM2PCMConfig[index].bit_depth = PDM2PCM_BITDEPTH_24IN32;
          break;
        default:
          break;
      }
  ...
  else
  {
    ...
    if (Instance == 0U)
    {
      ...
      if (HAL_SAI_Receive_DMA(&hAudioInSai, (uint8_t *)SAI_InternalBuffer,
                              (uint16_t)(AudioInCtx[Instance].Size / 2U * AudioInCtx[Instance].ChannelsNbr)) != HAL_OK)
      {
        return BSP_ERROR_PERIPH_FAILURE;
      }
#else
...
int32_t CCA02M2_AUDIO_IN_Resume(uint32_t Instance)
{
  ...
  else
  {
    ...
    else /* (Instance == 1U) */
    {
      ...
      for (counter = (int32_t)(AudioInCtx[Instance].ChannelsNbr); counter > 0; counter --)
      {
        if (HAL_DFSDM_FilterRegularStart_DMA(&hAudioInDfsdmFilter[counter - 1], MicRecBuff[counter - 1],
                                             DEFAULT_AUDIO_IN_BUFFER_SIZE) != HAL_OK)
        {
          return BSP_ERROR_PERIPH_FAILURE;
        }
      }
  ...
  else
  {
    ...
    for (i = 0; i < DFSDM_MIC_NUMBER; i++)
    {
      if ((mic_init[POS_VAL(audio_in_digital_mic)] != 1U)
          && ((AudioInCtx[Instance].Device & audio_in_digital_mic) == audio_in_digital_mic))
      {
        ...
        if (HAL_DFSDM_FilterRegularMsbStart_DMA(&hAudioInDfsdmFilter[POS_VAL(audio_in_digital_mic)],
                                                (int16_t *)pBuf[enabled_mic - 1U - pbuf_index], NbrOfBytes) != HAL_OK)
        {
          return BSP_ERROR_PERIPH_FAILURE;
        }
        else
...
int32_t CCA02M2_AUDIO_IN_RecordPDM(uint32_t Instance, uint8_t *pBuf, uint32_t NbrOfBytes)
{
  ...
  else
  {
    ...
    if (HAL_SAI_Receive_DMA(&hAudioInSai, (uint8_t *)pBuf,
                            (uint16_t)(NbrOfBytes / (AudioInCtx[Instance].BitsPerSample / 8U))) != HAL_OK)
    {
      return BSP_ERROR_PERIPH_FAILURE;
    }
    /* Update BSP AUDIO IN state */
...
static void DFSDM_FilterRegConvHalfCpltCallback(DFSDM_Filter_HandleTypeDef *hdfsdm_filter)
{
  ...
  else if (hdfsdm_filter == &hAudioInDfsdmFilter[POS_VAL(AUDIO_IN_DIGITAL_MIC1)])
  {
    for (j = 0; j < AudioInCtx[1].ChannelsNbr; j ++)
    {
      for (i = 0; i < (AudioInCtx[1].SampleRate / 1000); i++)
...
static void DFSDM_FilterMspInit(DFSDM_Filter_HandleTypeDef *hDfsdmFilter)
{
  ...
  for (i = 0; i < DFSDM_MIC_NUMBER; i++)
  {
    if ((mic_init[POS_VAL(AUDIO_IN_DIGITAL_MIC1)] != 1U)
        && ((AudioInCtx[1].Device & AUDIO_IN_DIGITAL_MIC1) == AUDIO_IN_DIGITAL_MIC1))
    {
      mic_num = POS_VAL(AUDIO_IN_DIGITAL_MIC1);
      mic_init[mic_num] = 1;
    }
    else if ((mic_init[POS_VAL(AUDIO_IN_DIGITAL_MIC2)] != 1U)
             && ((AudioInCtx[1].Device & AUDIO_IN_DIGITAL_MIC2) == AUDIO_IN_DIGITAL_MIC2))
    {
      mic_num = POS_VAL(AUDIO_IN_DIGITAL_MIC2);
      mic_init[mic_num] = 1;
    }
    else if ((mic_init[POS_VAL(AUDIO_IN_DIGITAL_MIC3)] != 1U)
             && ((AudioInCtx[1].Device & AUDIO_IN_DIGITAL_MIC3) == AUDIO_IN_DIGITAL_MIC3))
    {
      mic_num = POS_VAL(AUDIO_IN_DIGITAL_MIC3);
      mic_init[mic_num] = 1;
    }
    else if ((mic_init[POS_VAL(AUDIO_IN_DIGITAL_MIC4)] != 1U)
             && ((AudioInCtx[1].Device & AUDIO_IN_DIGITAL_MIC4) == AUDIO_IN_DIGITAL_MIC4))
    {
